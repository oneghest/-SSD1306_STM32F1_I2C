...
# include "string.h"
u8  font;
u8  OLED_PRINTF_Buffer[200];	//usb_printf发送缓冲区
u16 mx,my;
...
//返回：无
//OLED_ShowCHinese16*16(0,0,5,LOGO)//温
//============================================================== 
...
{
	OLED_Write_IIC_Command(I2C_Command);
}
void WriteDat(unsigned char I2C_Data)//写数据
{
	OLED_Write_IIC_Data(I2C_Data);
}
...
void display(void)  
{	
	OLED_ShowCHinese16x16(0,0,5,LOGO);//温
	OLED_ShowCHinese16x16(15,0,6,LOGO);//度
}
...
{  
	...
	va_list ap;
	va_start(ap,fmt);
	vsprintf((char*)OLED_PRINTF_Buffer,fmt,ap);
	va_end(ap);
	i=strlen((const char*)OLED_PRINTF_Buffer);//此次发送数据的长度
	for(j=0;j<i;j++)//循环发送数据
	{
		switch (font)
		{
			case font_6x8:
				c = OLED_PRINTF_Buffer[j] - 32;
			  if(OLED_PRINTF_Buffer[j]=='\n'){
					my++;
					mx=0;
				}
				else{
					...
					WriteCmd(0xb0+my);
					WriteCmd(((mx&0xf0)>>4)|0x10);
					WriteCmd((mx&0x0f)|0x01);
					for(k=0;k<6;k++)
						WriteDat(F6x8[c][k]);
					mx += 6;
				}
			...
			case font_8x16:
			  ...
			  if(OLED_PRINTF_Buffer[j]=='\n'){
					...
					WriteCmd((mx&0x0f)|0x01);
					for(k=0;k<8;k++)
						WriteDat(F8X16[c*16+k]);
					WriteCmd(0xb0+my+1);
					WriteCmd(((mx&0xf0)>>4)|0x10);
					WriteCmd((mx&0x0f)|0x01);
					for(k=0;k<8;k++)
						WriteDat(F8X16[c*16+k+8]);
					mx += 8;
				}
				break;
			...
			if(j==0)
				for(row=0;row<6;row++){
						WriteCmd(0xb0+my+row);
						WriteCmd((((mx-8)&0xf0)>>4)|0x10);
						WriteCmd(((mx-8)&0x0f)|0x01);
						for(k=0;k<8;k++)
							WriteDat(0x00);
				}
				
			  switch(OLED_PRINTF_Buffer[j]){
					case '\n':my+=2;mx=0;break;
					case '.':c=10;break;
					case 'c':c=12;break;
					case ' ':c=11;break;
					...
					case '7':
					case '8':
					case '9':c = OLED_PRINTF_Buffer[j] - 32 - 16;break;
					default:break;
				}
			  if(c==12){
					for(row=0;row<6;row++){
